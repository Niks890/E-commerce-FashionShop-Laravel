...
class InventoryController extends Controller
{
    ...
    public function store(Request $request,  CloudinaryService $cloudinaryService)
    {
        $data = $request->validate([
            ...
            'price.required' => 'Vui lòng điền giá nhập.',
            'price.min' => 'Giá tiền phải lớn hơn 0.',
            'colors.required' => 'Vui lòng nhập ít nhất một màu sắc cho sản phẩm.',
            'colors.*.required' => 'Vui lòng nhập màu sắc.',
            'sizes.required' => 'Vui lòng chọn ít nhất một kích cỡ cho sản phẩm.',
        ]);
        ...
        // Tính tổng số lượng và chuẩn bị dữ liệu cho inventory detail
        $totalQuantity = 0;
        $inventoryDetailsData = [];

        foreach ($request->colors as $colorIndex => $color) {
            $quantities = explode(',', $request->quantities[$colorIndex]);
            ...
            foreach ($quantities as $item) {
                if (!empty($item)) {
                    ...
                    // Lưu thông tin cho inventory detail
                    $inventoryDetailsData[] = [
                        'size' => $size,
                        'color' => $color,
        ...
        $uploadResult = $cloudinaryService->uploadImage($request->file('image')->getPathname(), 'inventory_images');
        if (isset($uploadResult['error'])) {
            return redirect()->back()->with('error', 'Upload ảnh thất bại: ' . $uploadResult['error']);
        }
        $product->image = $uploadResult['url']; // Lưu URL từ Cloudinary
        ...
        // Thêm các biến thể sản phẩm (ProductVariant)
        foreach ($request->colors as $colorIndex => $color) {
            $quantities = explode(',', $request->quantities[$colorIndex]);
            foreach ($quantities as $item) {
                if (!empty($item)) {
                    ...
                    ProductVariant::create([
                        'color' => $color,
                        'size' => $size,
                        'price' => $data['price'], // giá tự động đi cho ko bị sót
                        'stock' => (int)$quantity,
                        'product_id' => $product->id,
                    ]);
                }
        ...
        foreach ($inventoryDetailsData as $detail) {
            $sizes[] = $detail['size'] . '-' . $detail['quantity']  . '-' . $detail['color'];
        ...
        $inventoryDetail->price = $data['price'];
        $inventoryDetail->quantity = $totalQuantity;
        $inventoryDetail->size = implode(',', $sizes);
        $inventoryDetail->save();
        return redirect()->route('inventory.index')->with('success', "Thêm phiếu nhập mới thành công!");
    }
