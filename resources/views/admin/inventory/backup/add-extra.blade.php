        ...
        <div class="card shadow-lg mb-4 rounded-3">
            <div class="card-header bg-gradient-primary text-white py-3 rounded-top-3">
                <ul class="nav nav-tabs card-header-tabs">
                    <li class="nav-item">
                        <a class="nav-link active" id="existing-tab" data-bs-toggle="tab" href="#existing-products">
                            <i class="fas fa-box-open me-2"></i>Sản phẩm có sẵn
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" id="new-tab" data-bs-toggle="tab" href="#new-products">
                            <i class="fas fa-plus-circle me-2"></i>Sản phẩm mới
                        </a>
                    </li>
                </ul>
            </div>
            ...
            <div class="card-body tab-content">
                <!-- Tab sản phẩm có sẵn -->
                <div class="tab-pane fade show active" id="existing-products">
                    <div class="btn-group-bulk mb-4">
                        ...
                        <button type="button" class="btn btn-secondary rounded-pill px-4 shadow-sm" id="deselectAllProductsBtn">
                        ...
                        <table class="table table-bordered table-hover table-striped align-middle">
                            <thead class="table-light">
                                <tr>
                                    ...
                                    <th width="120px">Thông tin cơ bản</th>
                                    <th class="text-center">Biến thể hiện có</th>
                                    <th width="250px">Thông tin nhập thêm</th>
                                </tr>
                            </thead>
                            <tbody id="products-tbody">
                                <!-- Sản phẩm có sẵn sẽ được thêm bằng JS -->
                            </tbody>
                        </table>
                    </div>
                </div>
                ...
                <div class="tab-pane fade" id="new-products">
                    <div class="mb-4">
                        <button type="button" class="btn btn-primary rounded-pill px-4 shadow-sm" id="add-new-product">
                            <i class="fas fa-plus me-2"></i>Thêm sản phẩm mới
                        </button>
                    </div>

                    <div id="new-products-container">
                        <!-- Danh sách sản phẩm mới sẽ được thêm vào đây -->
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content border-0 shadow-lg">
                <div class="modal-header bg-primary text-white rounded-top-3">
                    <h5 class="modal-title fw-bold"><i class="fas fa-cube me-2"></i>Thông tin sản phẩm mới</h5>
                ...
                <div class="modal-body">
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <h5 class="fw-bold mb-0"><i class="fas fa-list-ul me-2"></i>Biến thể sản phẩm</h5>
                        <button type="button" class="btn btn-sm btn-outline-primary rounded-pill" id="add-new-variant">
                            <i class="fas fa-plus me-1"></i>Thêm biến thể
                        </button>
                    </div>

                    <div id="new-product-variants" class="mb-3">
                        <!-- Các biến thể sẽ được thêm vào đây -->
                        <div class="alert alert-info">
                            <i class="fas fa-info-circle me-2"></i>Thêm ít nhất một biến thể (màu sắc/size) cho sản phẩm
                        </div>
                    </div>
                </div>
                <div class="modal-footer border-top-0">
                    ...
                    <button type="button" class="btn btn-primary rounded-pill px-4 py-2" id="save-new-product">
...
@section('css')
    ...
    <style>
         ...
         /* Thêm style cho tab sản phẩm mới */
        .new-product-card {
...
@section('js')
 ...
 <script>
        // Thêm JS xử lý sản phẩm mới vào đây
        $(document).ready(function() {
            ...
            $('#add-new-variant').click(function() {
                const variantHtml = `
                    <div class="card variant-item mb-3">
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-5">
                                    <div class="mb-3">
                                ...
                                <div class="col-md-3">
                                    <div class="mb-3">
                                        ...
                                        <input type="number" class="form-control variant-quantity" min="1" value="1" required>
                                    </div>
                                </div>
                ...
                `;
                $('#new-product-variants .alert').remove();
                $('#new-product-variants').append(variantHtml);
            });

            // Xóa biến thể
            $(document).on('click', '.remove-variant', function() {
                $(this).closest('.variant-item').remove();
                if ($('#new-product-variants .variant-item').length === 0) {
                    $('#new-product-variants').html(`
                        <div class="alert alert-info">
                            <i class="fas fa-info-circle me-2"></i>Thêm ít nhất một biến thể (màu sắc/size) cho sản phẩm
                        </div>
                    `);
            ...
            $('#save-new-product').click(function() {
                ...
                const price = parseFloat($('#new-product-price').val());
                const variants = [];

                // Lấy thông tin các biến thể
                $('.variant-item').each(function() {
                    const color = $(this).find('.variant-color').val().trim();
                    const size = $(this).find('.variant-size').val();
                    const quantity = parseInt($(this).find('.variant-quantity').val());
                ...
                // Thêm vào danh sách sản phẩm mới
                newProductsData.push({
                    temp_id: tempId,
                    name: productName,
                    category_id: $('#new-product-category').val(),
                    brand: $('#new-product-brand').val().trim(),
                    price: price,
                    variants: variants,
                    image: $('#new-product-image')[0].files[0] ? $('#new-product-image')[0].files[0].name : null
                });

                // Hiển thị sản phẩm mới trong danh sách
                renderNewProduct(tempId, productName, variants, price);
            ...
            // Hiển thị sản phẩm mới trong danh sách
            function renderNewProduct(tempId, name, variants, price) {
                const variantsHtml = variants.map(v => `
                    <div class="d-inline-block me-2 mb-2">
                        <span class="badge bg-secondary variant-badge">${v.size}</span>
                        <span class="badge bg-primary variant-badge">${v.color}</span>
                        <span class="badge bg-success variant-badge">SL: ${v.quantity}</span>
                    </div>
                `).join('');

                const html = `
                    <div class="card new-product-card mb-3" data-temp-id="${tempId}">
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-start">
                                <div>
                                    <h6 class="fw-bold mb-2">${name}</h6>
                                    <div class="mb-2">
                                        <span class="badge bg-info me-2">Giá nhập: ${formatCurrency(price)}</span>
                                    </div>
                                    <div>${variantsHtml}</div>
                                </div>
                                <button type="button" class="btn btn-sm btn-outline-danger rounded-circle remove-new-product">
                                    <i class="fas fa-trash"></i>
                                </button>
                            </div>
                            <input type="hidden" name="new_products[]" value='${JSON.stringify({
                                temp_id: tempId,
                                name: name,
                                category_id: $('#new-product-category').val(),
                                brand: $('#new-product-brand').val().trim(),
                ...
                $('#new-products-container').append(html);
            }

            // Xóa sản phẩm mới
            $(document).on('click', '.remove-new-product', function() {
                const tempId = $(this).closest('.new-product-card').data('temp-id');
                newProductsData = newProductsData.filter(p => p.temp_id !== tempId);
                $(this).closest('.new-product-card').remove();
            });
            ...
            // Cập nhật form submit để gửi cả sản phẩm mới
            $('#formCreateInventory').on('submit', function(e) {
                // Thêm dữ liệu sản phẩm mới vào form
                if (newProductsData.length > 0) {
                    const newProductsInput = document.createElement('input');
                    newProductsInput.type = 'hidden';
                    newProductsInput.name = 'new_products';
                    newProductsInput.value = JSON.stringify(newProductsData);
                    this.appendChild(newProductsInput);
                }
            });
        });
    </script>
